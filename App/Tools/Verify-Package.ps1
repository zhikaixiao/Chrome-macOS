...
$customExes = Get-ChildItem -Path $rootDir -Filter '*.exe' -Recurse -File -ErrorAction SilentlyContinue | Where-Object {
    $_.FullName -notmatch '\\App\\Chrome-bin\\' -and
    $_.FullName -notmatch '\\App\\Tools\\7z(a)?\.exe' -and
    $_.FullName -notmatch '\\Release\\' -and
    $_.FullName -notmatch '\\ReleaseStaging\\'
}
if ($customExes.Count -gt 0) {
    $errors.Add("安全审计警告：在非 Chrome 目录检测到自定义 EXE 文件 ($($customExes.Name -join ', '))，必须保持纯脚本！")
} else {
    Write-Host "  - 安全审计通过：项目中零第三方 EXE 二进制，100% 纯脚本开源透明" -ForegroundColor Green
}

Write-Host '[2/5] 检查一键安装配置核心脚本...'
$setupScript = Join-Path $appDir 'Setup-Chrome.ps1'
$launchScript = Join-Path $appDir 'Launch-Chrome.ps1'
$batEntry = Join-Path $rootDir '一键安装配置.bat'

if (-not (Test-Path -LiteralPath $setupScript -PathType Leaf)) {
    $errors.Add('App\Setup-Chrome.ps1 未找到。')
} else {
    Write-Host "  - App\Setup-Chrome.ps1 正常" -ForegroundColor Green
}
if (-not (Test-Path -LiteralPath $launchScript -PathType Leaf)) {
    $errors.Add('App\Launch-Chrome.ps1 未找到。')
} else {
    Write-Host "  - App\Launch-Chrome.ps1 正常" -ForegroundColor Green
}
...
$configPath = Join-Path $appDir 'Config\extensions.json'
if (-not (Test-Path -LiteralPath $configPath -PathType Leaf)) {
    $errors.Add('Config\extensions.json 未找到。')
} else {
    $config = Get-Content -LiteralPath $configPath -Raw -Encoding UTF8 | ConvertFrom-Json
    foreach ($extension in $config.extensions) {
        $manifestPath = Join-Path (Join-Path $appDir 'Extensions') (Join-Path $extension.folder 'manifest.json')
...
# 1. 确认无任何内置的第三方侵权脚本
$violentmonkeyDir = Join-Path $appDir 'Extensions\Violentmonkey'
...
$ubolDir = Join-Path $appDir 'Extensions\uBOLite'
...
try {
    ...
    if ($resp.StatusCode -eq 'OK') {
        Write-Host "  - 官方下载源直连连通正常 (HTTP 200 OK, 大小: $([math]::Round($resp.ContentLength/1MB, 2)) MB)" -ForegroundColor Green
    }
    $resp.Close()
} catch {
