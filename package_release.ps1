...
$stagingParent = Join-Path $rootDir 'ReleaseStaging'
$stagingDir = Join-Path $stagingParent 'GoogleChrome-Portable'
$releaseOutDir = Join-Path $rootDir 'Release'

Write-Host "================================================================================" -ForegroundColor Cyan
Write-Host "     Building Google Chrome Portable Release Package (ZIP format)              " -ForegroundColor Yellow
Write-Host "================================================================================" -ForegroundColor Cyan
Write-Host ""

# 1. Clean previous staging & release
...
# 2. Copy source files
Write-Host "[1/3] Copying files to staging directory..." -ForegroundColor Cyan
Copy-Item -Path "$sourceDir\*" -Destination $stagingDir -Recurse -Force

# 3. Optimize script encoding with UTF-8 BOM
Write-Host "[2/3] Checking script encoding..." -ForegroundColor Cyan
...
# 4. Compress into ZIP
Write-Host "[3/3] Compressing release package into ZIP..." -ForegroundColor Cyan

# Clean old zip files
Get-ChildItem -Path $releaseOutDir -Filter '*.zip*' | Remove-Item -Force -ErrorAction SilentlyContinue

$zipPath = Join-Path $releaseOutDir 'GoogleChrome-Portable.zip'
...
Write-Host "    [OK] Google Chrome Portable ZIP package generated successfully!             " -ForegroundColor Green
Write-Host "================================================================================" -ForegroundColor Green
