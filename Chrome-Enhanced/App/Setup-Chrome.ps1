...
# Enhanced edition | 3 Open-source extensions | Isolated user data | Bing default
# ==============================================================================
...
function Configure-ChromeDefaults {
    ...
    $preferencesObj = @{
        ...
        extensions = @{
            pinned_extensions = @(
                "albmclihjpfalbbkfdmdkagmjnbnapmd",
                "pegfhijceedfakgcpeiomonohpfbdjfc",
                "emhpahaelahindohfhmmgpemmhknccha"
            )
            toolbar = @(
                "albmclihjpfalbbkfdmdkagmjnbnapmd",
                "pegfhijceedfakgcpeiomonohpfbdjfc",
    ...
    Write-Host "[+] Default settings configured successfully (Bing search / zh-CN locale / Home button / pinned extensions)." -ForegroundColor Green
}
...
function Create-DesktopAndLocalShortcuts {
    ...
    $targetPath = Join-Path $script:RootDir 'Start-Chrome.bat'
    $launcherExe = Join-Path $script:AppDir 'Launcher.exe'
    if (Test-Path $launcherExe) {
        $targetPath = $launcherExe
    }

    $workDir = $script:RootDir
    $desc = "Google Chrome (Portable)"

    try {
        [NativeShortcutHelper]::CreateShortcut($shortcutPath, $targetPath, "", $workDir, $desc, $ChromePath, 0)
...
# 确保 Launcher.exe 存在
$launcherExe = Join-Path $script:AppDir 'Launcher.exe'
$launcherSrc = Join-Path $script:AppDir 'Tools\ChromeLauncher.cs'
...
# 安装结束立即启动浏览器与插件
Write-Host "[*] Launching Google Chrome directly with Bing homepage and extensions..." -ForegroundColor Green

if (Test-Path $launcherExe) {
    Start-Process -FilePath $launcherExe
} else {
    powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File (Join-Path $script:AppDir 'Launch-Chrome.ps1')
}
